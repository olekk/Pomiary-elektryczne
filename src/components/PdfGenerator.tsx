...
export const PdfGenerator: React.FC<PdfGeneratorProps> = ({ inspection }) => {
  const passedCount = inspection.measurements.filter(m => m.result === 'TAK').length;
  const failedCount = inspection.measurements.filter(m => m.result === 'NIE').length;
  const noGroundingCount = inspection.measurements.filter(m => m.result === 'B.UZ').length;
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>Pomiar impedancji pętli zwarciowej</Text>
        </View>
