...
describe('generateProtocolNumber', () => {
  ...
  it('puts the address first, so protocols sort by building', () => {
    const date = new Date(2026, 5, 15)
    const result = generateProtocolNumber(date, '42', 'ul. Testowa 1')

    expect(result.split('/')[0]).toBe('TESTOWA_1')
    expect(result.split('/').at(-1)).toBe('PROT')
  })
