...
interface Inspection {
  ...
  izolacjaData?: IzolacjaData // only unitType === 'mieszkanie'; absent on protocols created before this section
}
...
`IzolacjaData` (insulation-resistance section of a `mieszkanie` protocol) holds `ukladSieci` (`TN-C`/`TN-S`), `materialPrzewodow` (`Cu`/`Al`), `napiecieProbiercze` (`'500'` default / `'250'`) and `obwody: { id, rodzaj, nazwaInny?, ocena }[]` — one row per circuit, duplicates allowed, min. 1 row, `nazwaInny` only when `rodzaj === 'inny'` (≤ 40 chars). No measured values are stored: the required resistance is the constant `R_IZOLACJI_WYMAGANA` (1,0 MΩ) and the technician only records `w-normie` / `ponizej-normy`. `createIzolacjaData()` prefills TN-C, Cu, 500 V and oświetlenie + gniazda rated `w-normie`, and a newly added row gets `w-normie` too — the technician changes only what differs. "Not chosen yet" is `null` (never `undefined`) — in practice a newly added row's circuit type — and is only legal in a draft — `validateIzolacja()` blocks save, and `canGeneratePdf()` blocks PDF generation of an incomplete section. Helpers are in `utils/izolacja.ts`, option labels in `constants/izolacja.ts`, UI in `organisms/IzolacjaInspectionForm.tsx`, PDF in `components/IzolacjaPdfSection.tsx`. **Migration**: a document without `izolacjaData` is valid — the section is omitted from the form, summary and PDF, and the verdict falls back to the Zs rule.
...
**Protocol verdict**: `getProtocolVerdict(inspection)` (`utils/protocolVerdict.ts`) is the single source of the protocol conclusion — dwellings derive it from measurements (any `NIE` → `nie-nadaje`) combined worst-wins with `izolacjaData` (any circuit `ponizej-normy` → `nie-nadaje`), `klatka` reads `klatkaData.ocenaInstalacji`, `odgromowa` reads `odgromowaData.wynik` (missing → `nadaje`). `getNextInspectionDate()` returns +5 years unless the verdict is `nie-nadaje`. PDF, `SummaryScreen` and both forms consume these; don't re-derive the verdict inline.
...
2. **`MeasurementScreen`** holds it in `useState` and mirrors every change to `sessionStorage` (key: `draft-inspection:{buildingId}`) via an `updateInspection()` wrapper. The identity fields (adres / typ lokalu / numer / właściciel) are edited **inline at the top of the screen** — they write into the same in-memory inspection through `updateInspection`. The screen also subscribes to sibling inspections (`useCollection`) to drive the duplicate-number warning and automatic `klatka` numbering. Its header carries **Anuluj** (clears the draft → back to building) and **Niedostępne** (fire-and-forget save of an `INACCESSIBLE` record → back; hidden when resuming an existing unit). It only writes to Firestore when the user taps "Zapisz," using a **client-generated ID** (`generateInspectionId()`) so the write never blocks navigation; the protocol number is (re)generated at save/inaccessible time from the possibly-edited apartment number.
   ...
   - **`izolacjaData`** (mieszkanie only) lives _inside_ the in-memory inspection, so unlike `klatkaData`/`odgromowaData` it is mirrored to the `sessionStorage` draft. `shouldStartIzolacja()` seeds it (TN-C, Cu, 500 V, oświetlenie + gniazda rated `w-normie`) for a new or previously-`INACCESSIBLE` mieszkanie — at init and when the type selector is switched to Mieszkanie; completed protocols from before the section are edited without it. "Zapisz" is blocked while `validateIzolacja()` reports errors — errors are shown next to each field (after the first save attempt) and the screen scrolls to the section. "Niedostępne" saves without it. `saveInspectionToFirestore` builds the payload explicitly (`toIzolacjaPayload`, the array is nested so the shallow `withoutUndefinedFields` isn't enough) and writes `deleteField()` for non-mieszkanie types, since the `merge` write would otherwise leave stale data after a type change.
...
- An inspection with an incomplete `izolacjaData` (`canGeneratePdf()` false) is not rendered: the single-PDF path shows an error toast, the batch path counts it as failed.
...
- `PdfGenerator` branches its body by unit type: dwellings → Zs table + (mieszkanie with `izolacjaData`) `IzolacjaPdfSection` + automatic remarks per failed circuit (`getIzolacjaRemarks()`, appended to the Zs auto-remarks, not stored in `notes`) + PN-IEC 60364-6-61 oględziny + owner clause/signature; `klatka` → checklist; `odgromowa` → `OdgromowaPdfSection` (sections 2–9, incl. an empty frame for the building sketch, which is not implemented yet). Title, conclusion text/colour (`VERDICT_CONCLUSIONS`) and next-inspection date all come from `getProtocolVerdict()`.
...
| `IzolacjaInspectionForm`                             | organisms | Mieszkanie insulation resistance: network/conductor/test voltage + circuits table    |
