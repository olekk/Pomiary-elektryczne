...
## Era 15 — Maintenance sprint: mappers centralized, quality gates repaired (2026-07-03)

A dedicated cleanup pass with no feature changes:

- **Firestore document mappers centralized** into `utils/firestoreMappers.ts` (unit-tested). Previously `inspectionMapper`/`buildingMapper` were independently redefined in `ProjectsScreen`, `ProjectDetailsScreen`, `BuildingDetailsScreen`, `MeasurementScreen`, `SummaryScreen`, and `usePendingSync`, with diverging field coverage — `ProjectDetailsScreen` and `usePendingSync` didn't map `klatkaData`, and `usePendingSync` lacked the legacy `technician`/`signature` fallbacks and needed an `as Inspection` cast. The unified mapper is the superset of all copies, so a couple of sites gained fields they previously dropped (no behavioral impact found: the project-level list only uses status/buildingId; the pending-sync retry writes with `merge: true`, so omitted fields were preserved server-side).
- **Quality gates repaired.** `npm test` had 8 failures — `protocolGenerator.test.ts` still asserted the pre-2026-06-15 date-first protocol format (the code and its header comment were also inconsistent; the comment claimed the old format). `npm run lint` had 9 errors (new react-hooks v7 rules, a stray `any` in `ensureDate`, a use-before-declaration in `SummaryScreen`, stale disable directives). `npm run test:coverage` was failing its own 90% threshold because browser-only modules (`toast.ts`, `generatePdf.tsx`) sit in the covered `utils/` glob; they're now explicitly excluded (they can't run in the node test environment) and `cn.ts`/`logger.ts` gained tests. The "latest ref" pattern in `useCollection`/`useDocument` moved its ref writes from render into an effect to satisfy `react-hooks/refs`; remaining sync-from-external-store `setState` calls carry targeted, commented `eslint-disable` lines rather than being restructured — those hooks are the offline-critical core and not worth churn for a lint rule.
- **Dead code removed**: `molecules/StatusBadge.tsx` (only referenced from a commented-out block in `MainLayout`), `src/App.css`, `src/assets/react.svg`. The accidentally-committed `reports/mutation/mutation.html` was untracked, and `reports/`/`coverage/` added to `.gitignore` + ESLint ignores.
- **Small extractions**: the floating action button (three verbatim copies) → `atoms/Fab.tsx`; the cache-freshness chip (duplicated between `InspectionsList` and `ProjectDetailsScreen`) → `molecules/DataSourceChip.tsx`.
- **`atoms/Badge.tsx` moved to the dark palette** (`bg-emerald-900/50 text-emerald-300` etc.), closing the long-standing §16 light-mode-colors inconsistency; its previous `border-*` color classes were inert (no `border` width class), so it now also gets the same visible border as `DataSourceChip`.
- **Docs corrected against source**: `ARCHITECTURE.md` §8 claimed an `online`-event sync-retry trigger that doesn't exist in the current code (the `online` listener only feeds the `useOnlineStatus` UI hint; retry fires on `MainLayout` mount). The claim was corrected and the missing trigger recorded as known debt rather than silently (re)added, since it's a sync-behavior change.
- `console.error` calls in screens/hooks unified to `logger.error` (behavior-identical passthrough; matches the documented write-path convention).
